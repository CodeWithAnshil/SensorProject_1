...
class PredictionPipelineConfig:
  ...
  prediction_file_name:str = "prediction_file_.csv"
...
class PredictionPipeline:
  ...
  def get_predicted_datafram(self, input_datafram_path:pd.DataFrame):
    try:

      prediction_column_name:str = TARGET_COLUMN

      input_dataframe: pd.DataFrame = pd.read_csv(input_datafram_path)

      input_dataframe = input_dataframe.drop(columns="Unnamed: 0") if "Unnamed: 0" in input_dataframe.columns else input_dataframe

      preditions = self.predict(input_dataframe)

      input_dataframe[prediction_column_name] = [pred for pred in preditions]
  ...
  def run_pipeline(self):
    try:
      ...
      self.get_predicted_datafram(input_csv_path)
